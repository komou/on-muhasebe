...
import { irsaliyeListesi, irsaliyeFaturalandir } from './Irsaliyeler';
import { urunListesi_global } from './Urunler';
import { satiseFaturaCari } from './veriAkisi';
...
const durumRenk = {
  ...
  'Gönderildi': { bg: '#dbeafe', renk: '#1d4ed8' },
  'Ödendi': { bg: '#dcfce7', renk: '#166534' },
  'İptal': { bg: '#fee2e2', renk: '#991b1b' },
};

export let satisFaturaListesi = [];

function SatisFaturalari() {
  const [faturalar, setFaturalar] = useState([]);
  const [yukleniyor, setYukleniyor] = useState(true);
  const [arama, setArama] = useState('');
  const [durumFiltre, setDurumFiltre] = useState('');
  const [formAcik, setFormAcik] = useState(false);
  const [yeni, setYeni] = useState({ cari: '', tarih: '', tip: '', vade: '', notlar: '' });
  const [seciliIrsaliyeler, setSeciliIrsaliyeler] = useState([]);
  const [satirlar, setSatirlar] = useState([{ kod: '', urun: '', miktar: 1, fiyat: '', kdv: 20 }]);
  ...
  const faturalariYukle = async () => {
    ...
    if (error) {
      ...
      const donusturulmus = data.map(f => ({
        id: f.id, no: f.no, tarih: f.tarih,
        cari: f.cari, tip: f.tip,
      ...
      satisFaturaListesi.splice(0, satisFaturaListesi.length, ...donusturulmus);
    }
    setYukleniyor(false);
  };

  const cariSec = (unvan) => {
    const bulunan = cariListesi.find(c => c.unvan === unvan);
    const tip = bulunan?.eFaturaMukellef ? 'e-Fatura' : 'e-Arşiv';
    setYeni({ ...yeni, cari: unvan, tip });
    setSeciliIrsaliyeler([]);
    setSatirlar([{ kod: '', urun: '', miktar: 1, fiyat: '', kdv: 20 }]);
  };

  const cariIrsaliyeleri = irsaliyeListesi.filter(i =>
    i.cari === yeni.cari && !['Faturalandı', 'İptal'].includes(i.durum)
  );

  const irsaliyeToggle = (irsNo) => {
    const irsaliye = irsaliyeListesi.find(i => i.no === irsNo);
    if (!irsaliye) return;
    if (seciliIrsaliyeler.includes(irsNo)) {
      ...
      const yeniSatirlar = irsaliye.satirlar.map(s => {
        const urun = urunListesi_global.find(u => u.ad === s.urun || u.kod === s.kod);
        return {
      ...
      setSatirlar(prev => [...prev.filter(s => s.urun !== ''), ...yeniSatirlar]);
    }
  };

  const satirEkle = () => setSatirlar([...satirlar, { kod: '', urun: '', miktar: 1, fiyat: '', kdv: 20 }]);
  const satirSil = (i) => setSatirlar(satirlar.filter((_, idx) => idx !== i));
  const satirGuncelle = (i, alan, deger) => {
    const y = [...satirlar];
    y[i][alan] = deger;
    if (alan === 'urun') {
      const bulunan = urunListesi_global.find(u => u.ad === deger);
      if (bulunan) { y[i].fiyat = bulunan.satisFiyat || ''; y[i].kdv = bulunan.kdv; y[i].kod = bulunan.kod; }
    }
    setSatirlar(y);
  };
  ...
  const kaydet = async () => {
    ...
    const { count } = await supabase.from('satis_faturalari').select('*', { count: 'exact', head: true });
    const no = `FAT-2026-${String((count || 0) + 1).padStart(3, '0')}`;

    // İrsaliyeleri faturalandı yap
    for (const irsNo of seciliIrsaliyeler) {
      await irsaliyeFaturalandir(irsNo);
    }

    // Cari bakiyesini güncelle
    await satiseFaturaCari(yeni.cari, toplam, no);

    const { error } = await supabase.from('satis_faturalari').insert({
      no,
    ...
    setYeni({ cari: '', tarih: '', tip: '', vade: '', notlar: '' });
    setSeciliIrsaliyeler([]);
    setSatirlar([{ kod: '', urun: '', miktar: 1, fiyat: '', kdv: 20 }]);
    setFormAcik(false);
    alert(`✅ Fatura kaydedildi!${seciliIrsaliyeler.length > 0 ? `\n${seciliIrsaliyeler.join(', ')} irsaliyeleri faturalandı.` : ''}`);
  };

  const durumDegistir = async (id, durum) => {
    await supabase.from('satis_faturalari').update({ durum }).eq('id', id);
    await faturalariYukle();
  };

  const filtrelenmis = faturalar.filter(f => {
    const aramaUygun = f.cari.toLowerCase().includes(arama.toLowerCase()) || f.no.toLowerCase().includes(arama.toLowerCase());
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', gap: '12px' }}>
        <div style={{ display: 'flex', gap: '12px' }}>
          <input placeholder="🔍 Fatura veya cari ara..." value={arama} onChange={e => setArama(e.target.value)} style={{ ...inputStil, width: '280px' }} />
      ...
      {formAcik && (
        <div style={kartStil}>
          <h3 style={{ margin: '0 0 20px', fontSize: '16px', borderBottom: '1px solid #e2e8f0', paddingBottom: '12px' }}>📄 Yeni Satış Faturası</h3>
          ...
          <div style={{ background: '#f8fafc', borderRadius: '8px', padding: '16px', marginBottom: '16px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px' }}>
              ...
              <div>
                <label style={labelStil}>Fatura Tipi</label>
                <div style={{ padding: '10px', background: yeni.tip === 'e-Fatura' ? '#dcfce7' : '#f1f5f9', borderRadius: '8px', border: '1px solid #e2e8f0', fontSize: '14px', color: yeni.tip === 'e-Fatura' ? '#166534' : '#475569', fontWeight: '600' }}>
                  {yeni.tip || '— Cari seçince otomatik gelir —'}
                </div>
              </div>
              <div>
          ...
          {yeni.cari && (
            <div style={{ background: '#f8fafc', borderRadius: '8px', padding: '16px', marginBottom: '16px' }}>
              ...
              {cariIrsaliyeleri.length === 0
                ? <p style={{ fontSize: '13px', color: '#94a3b8', margin: 0 }}>Bu cariye ait faturalanmamış irsaliye yok</p>
                : (
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                    {cariIrsaliyeleri.map(irs => (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
              <thead>
                <tr style={{ borderBottom: '2px solid #e2e8f0' }}>
                  {['Kod', 'Ürün / Hizmet', 'Miktar', 'Birim Fiyat (₺)', 'KDV %', 'Tutar (₺)', 'Kaynak', ''].map(b => (
                ...
                {satirlar.map((s, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid #e2e8f0', background: s.kaynakIrs ? '#f0f9ff' : 'white' }}>
                    <td style={{ padding: '8px', width: '100px' }}>
                      <input placeholder="Kod" value={s.kod} onChange={e => satirGuncelle(i, 'kod', e.target.value)} style={inputStil} />
                    </td>
                    <td style={{ padding: '8px' }}>
                      <select value={s.urun} onChange={e => satirGuncelle(i, 'urun', e.target.value)} style={inputStil}>
                    ...
                    <td style={{ padding: '8px', width: '120px', fontWeight: '600', color: '#22c55e' }}>
      ...
      {yukleniyor ? (
        ...
        <div style={kartStil}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
            <thead>
              <tr style={{ borderBottom: '2px solid #e2e8f0' }}>
                {['Fatura No', 'Tarih', 'Cari', 'Tip', 'İrsaliyeler', 'Tutar', 'KDV', 'Toplam', 'Durum', 'İşlem'].map(b => (
            ...
            <tbody>
              {filtrelenmis.map(f => (
                <tr key={f.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                  ...
                  <td style={{ padding: '12px' }}>{f.cari}</td>
                  <td style={{ padding: '12px' }}>
                    <span style={{ padding: '3px 8px', borderRadius: '20px', fontSize: '11px', background: f.tip === 'e-Fatura' ? '#dcfce7' : '#f1f5f9', color: f.tip === 'e-Fatura' ? '#166534' : '#475569' }}>{f.tip}</span>
                  </td>
                  <td style={{ padding: '12px' }}>
                  ...
                  <td style={{ padding: '12px', color: '#64748b' }}>₺{f.kdv.toLocaleString()}</td>
                  <td style={{ padding: '12px', fontWeight: '600' }}>₺{f.toplam.toLocaleString()}</td>
                  <td style={{ padding: '12px' }}>
                    <span style={{ padding: '3px 10px', borderRadius: '20px', fontSize: '12px', background: durumRenk[f.durum]?.bg, color: durumRenk[f.durum]?.renk }}>{f.durum}</span>
                  </td>
                  <td style={{ padding: '12px' }}>
